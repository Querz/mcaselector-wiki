...
* There are two selection modes. When zooming in until the chunk grid can be seen, individual chunks can be 
  selected or deselected. When zooming out until only the region grid is visible, entire regions can be selected 
  or deselected.
...
* Holding `Shift` turns the cursor into a paint brush.
* To select *everything*, clear the selection (`Selection --> Clear`) and invert the selection (`Selection --> Invert`).
...
* A selection can also be inverted on a per-region basis *only* if a region contains any form of selection (either the 
  entire region is selected or at least one chunk in the region is selected). Use `Selection --> Invert selected regions`
  to achieve this. This is *not* a toggle, so inverting a fully selected region will fully deselect it, and it will not be 
...
* When a full region is selected, it is represented by a single line only containing the region coordinate. E.g. when 
  selecting the region `5 | -3` the line would only contain `5;-3`.
...
* *Defragment* the selected regions. This rewrites the region files to get rid of any empty sectors to save space. It
  does not change any of the chunk data.
...
* Edit this chunk's data manually (see [Chunk Editor](Chunk-Editor)).
* The [NBT Changer](NBT-Changer) shows this chunk's values as a preview for some fields.

### Selecting exactly two chunks

* Using `Tools --> Swap chunks`, the two selected chunks can be swapped. This can be useful when chunk 
